 ...
 * <p>
 * Left click is the bookmark action: it marks the hovered position in JourneyMap (see {@link #createWaypoint}).
 * Right click broadcasts the chunk under the cursor: the report goes to the server, which puts it into the public
 * chat in the name of the clicker, so every player on the server sees who reported which chunk.
 */
...
public class ScanMapWidget extends Widget<ScanMapWidget> implements Interactable {
    ...
    /** Left click bookmarks the hovered position, right click broadcasts the chunk it belongs to. */
    @Override
    ...
    /**
     * T teleports to the hovered cell. The scan GUI is closed right away: the map shows a scan of the area the
     * player is leaving, and closing also drops the mode selection GUI underneath it. Only offered when the server
     * allowed the teleport for this scan (cheat mode + config), which is also what the tooltip line follows.
     */
    @Override
    public Interactable.Result onKeyPressed(char typedChar, int keyCode) {
        if (keyCode != Keyboard.KEY_T || !state.teleportAllowed()) return Interactable.Result.IGNORE;
        updateHover(getContext());
        if (!hoverInside) return Interactable.Result.IGNORE;
        CommonProxy.simpleNetworkWrapper.sendToServer(
    ...
    /**
     * The bookmark action of the map, i.e. the left click: it marks this position in JourneyMap, which is what the
     * bookmark hint in the tooltip promises. The bedrock modes and dense_and_normal write their waypoints
     * straight into JourneyMap - one per material the scan found in the chunk, in that material's own colour - while
     * every other mode opens JourneyMap's waypoint editor, exactly like a double click on its fullscreen map (its
     * default name is then the coordinates of the cell). The exit position is resolved by the server, so the
     * waypoints sit on the very block a teleport would use even when the client has not loaded the chunk.
     */
