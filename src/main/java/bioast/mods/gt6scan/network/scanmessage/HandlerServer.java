...
import bioast.mods.gt6scan.utils.GT5Ores;
...
public class HandlerServer implements IMessageHandler<ScanRequest, ScanResponse> {
    ...
    private static final class ScanJob implements ScanScheduler.Job {
        ...
        /** Cache of {@link #gt5OreID}, same key as {@link #oreDictCache}; empty and unused without GT5. */
        private final HashMap<Integer, Short> gt5Cache = new HashMap<>();
        ...
        private void scanChunk(Chunk chunk) {
            ...
            scanTileEntities(chunk);
            // GT5's ores are plain blocks without a tile entity, so the reserved GT5 compatibility is a block pass on
            // top of the tile entity pass - for the two ore modes it applies to, and only when GT5 really is
            // installed (see GT5Ores), so a normal GT6 only install never pays for any of it
            if (GT5Ores.isPresent() && (mode == ScanMode.LARGE || mode == ScanMode.SMALL)) scanBlocks(chunk);
        }
         ...
         * because every placed ore block is its own tile entity - the block pass additionally runs for LARGE/SMALL to
         * pick up the ores of GT5, whose ore blocks carry no tile entity (see {@link GT5Ores}).
         */
        ...
        private short oreIDForMode(Block block, int meta) {
            ...
            // LARGE/SMALL only get here for the ores of GT5, GT6's own ones of those modes are tile entities
            if (mode == ScanMode.LARGE || mode == ScanMode.SMALL) return gt5OreID(block, meta, mode == ScanMode.SMALL);
        ...
        /**
         * GT5's ores, counted by LARGE and SMALL next to GT6's own ones, split the way detrav's prospector splits
         * them: LARGE takes the normal ores, SMALL the small ores. They cannot be read through the ore dictionary
         * like the ores of the other mods, GT6 does not know the prefixes of GT5's stone types ("oreNetherrack",
         * "oreBlackgranite", ...) and resolves those names to nothing, so they are read through GT5's own block
         * class instead - which is what {@link GT5Ores} does, and why that class is guarded by reflection.
         */
        private short gt5OreID(Block block, int meta, boolean small) {
            int key = (Block.getIdFromBlock(block) << 16) | (meta & 0xFFFF);
            Short cached = gt5Cache.get(key);
            if (cached != null) return cached;
            short matID = GT5Ores.oreID(block, meta, small);
            gt5Cache.put(key, matID);
            return matID;
        }
